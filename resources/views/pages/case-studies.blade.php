    ...
    <div class="wrap">
      ...
      <div class="filters reveal" role="group" aria-label="Filter case studies by engagement type">
        <button class="filter is-active" type="button" data-filter="all" aria-pressed="true">
          <span>All</span>
          <span class="filter__count">{{ $caseStudies->count() }}</span>
        </button>
        @foreach ($categories as $key => $label)
          <button class="filter" type="button" data-filter="{{ $key }}" aria-pressed="false">
            <span>{{ $label }}</span>
            <span class="filter__count">{{ $caseStudyCounts->get($key, 0) }}</span>
          </button>
        @endforeach
      </div>
      ...
      <div class="cases__grid" id="caseGrid">
        ...
        @foreach ($caseStudies as $caseStudy)
          <article class="case-card reveal" data-cat="{{ $caseStudy->category }}" data-delay="{{ ($loop->index % 3) * 90 }}">
            <div class="case-card__top">
              <span class="case-card__index">{{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }}</span>
              <span class="tag tag--{{ $caseStudy->category }}">{{ $caseStudy->categoryLabel() }}</span>
            </div>
            <p class="case-card__sector">{{ $caseStudy->sector }}</p>
            <h2 class="case-card__title"><a class="case-card__link" href="#">{{ $caseStudy->title }}</a></h2>
            <p class="case-card__metric"><span class="case-card__metric-val">{{ $caseStudy->metric_value }}</span><span class="case-card__metric-lab">{{ $caseStudy->metric_label }}</span></p>
            <p class="case-card__outcome">{{ $caseStudy->outcome }}</p>
            <div class="case-card__foot">
              <span class="case-card__meta">{{ $caseStudy->year_range }} · {{ $caseStudy->duration }}</span>
